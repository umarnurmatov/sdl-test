...
void Vertex::applyForce(Vec2f &force)
{
...
void Edge::update()
{
    Vec2f v1v2 = v2->position - v1->position;
    float diff = v1v2.len() - length;
...
std::pair<float, float> ConvexPolygon::ProjectToAxis(Vec2f &axis) const
{
    float min, max;
    float dot = dot_product(axis, (*vertexes.begin()).position);
    min = max = dot;

    for(auto& vertex : vertexes)
    {
        dot = dot_product(axis, vertex.position);
        min = std::min(min, dot);
        max = std::max(max, dot);
    }
...
void ConvexPolygon::calculateMassCenter_andBoundingBox()
{
    ...
    boundBox = FloatRect(min, max - min);
...
bool Solver::detectCollision(ConvexPolygon *b1, ConvexPolygon *b2)
{
    ...
    for(size_t i = 0; i < b1->edges.size() + b2->edges.size(); i++)
    {
        ...
        Vec2f axis = e->v2->position - e->v1->position;
...
{
    Vec2f collisionVector = m_collisionInfo.normal * m_collisionInfo.depth;
